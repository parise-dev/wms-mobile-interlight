...
@Component({
  selector: 'app-volumes-new',
  templateUrl: './volumes-new.page.html',
  styleUrls: ['./volumes-new.page.scss'],
})

export class VolumesNewPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.ids = localStorage.getItem('absEntry')?.toString() ?? ''
    this.userid = JSON.parse(localStorage.getItem('authorize') ?? '').userid

    this.service.getDisponiveisWithPack(localStorage.getItem('absEntry'), JSON.parse(localStorage.getItem('authorize') ?? '').userid).subscribe((data) => {
      data.forEach(i =>{
  ...
  async addPacking() {
    ...
    this.service.createPackList(this.addNEwPacking).subscribe(
      ...
      this.setOpen(true)
      this.spinner.hide();
      this.navCtrl.navigateForward(`/pick-item/${this.ids}/${this.userid}`, { replaceUrl: true });
